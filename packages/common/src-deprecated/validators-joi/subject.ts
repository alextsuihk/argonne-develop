 ...
 */
const add = Joi.object().keys({ action, name, remark });
...
const update = Joi.object().keys({ id, action, name, remark });
const addLevel = Joi.object().keys({ id, action, levelId, remark });
const removeLevel = Joi.object().keys({ id, action, levelId, remark });

export default { add, find, findOne, remove, update, addLevel, removeLevel };
