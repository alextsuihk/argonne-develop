...
const bind = async (req: Request, args: unknown): Promise<StatusResponse> => {
  ...
  if (bindingPrefix !== TENANT_BINDING_TOKEN_PREFIX || !tenantId || refreshPrefix !== REFRESH_TOKEN || !userId)
  ...
  await Promise.all([
    DatabaseEvent.log(userId, `/tenant-binding/${userId}`, 'BIND', { tenantId, studentId }),
    notifySync(
      user.tenants[0] || null,
      { userIds: [userId], event: 'AUTH-RENEW-TOKEN' },
      {
        bulkWrite: {
          users: [
            { updateOne: { filter: { _id: userId, tenants: { $ne: tenant._id } }, update } },
...
const unbind = async (req: Request, args: unknown): Promise<StatusResponse> => {
  ...
  const user = await User.findOneAndUpdate({ _id: userId, tenants: tenant._id }, update, { new: true }).lean();
  if (!user) throw { statusCode: 422, code: MSG_ENUM.USER_INPUT_ERROR };
