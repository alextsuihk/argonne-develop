...
describe(`${route.toUpperCase()} API Routes`, () => {
  ...
  test('should pass when creating (requesting) a presignedUrl', async () => {
    ...
    expect(res.body).toEqual({ data: { url: expect.any(String), expiry: expect.any(Number) } });
    ...
    expect(res2.body).toEqual({ data: { url: expect.any(String), expiry: expect.any(Number) } });
